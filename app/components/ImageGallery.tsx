...
export default function ImageGallery({ images, cityTitle }: ImageGalleryProps) {
  ...
  const getSizes = (imageCount: number) => {
    ...
    if (imageCount <= 4) return '(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 50vw';
    if (imageCount <= 6) return '(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw';
  ...
  return (
    <div className="mb-6">
      <h3 className="text-xl font-bold text-gray-800 mb-4 font-serif">
        Galería de Recuerdos ({images.length} {images.length === 1 ? 'imagen' : 'imágenes'})
      </h3>
      ...
      <div className={`grid gap-4 ${getGridLayoutClass(images.length)}`}>
        {images.map((imageUrl, index) => (
          <div
            key={index}
            className={`relative overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 group cursor-pointer ${getImageAspectRatio(images.length)}`}
